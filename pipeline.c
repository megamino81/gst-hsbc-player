...
int main(int argc, char *argv[]) 
{
    ...
    data.video_sink = gst_element_factory_make("waylandsink","video_sink");
    ...
    caps = gst_caps_new_simple( "video/x-h264, parsed=(boolean)false",
    ...
    !data.audio_sink || !data.videoqueue || !data.video_decoder || !data.video_convert || !data.video_sink || !data.avc_parser) {
    g_printerr ("Not all elements could be created.\n");
    return -1;
    }

    gst_bin_add_many (GST_BIN (data.pipeline), data.source,data.demuxer,data.audioqueue,data.audio_decoder,data.audio_convert,data.audio_sink,data.videoqueue,data.avc_parser, data.video_decoder,data.video_convert,data.video_sink, NULL);
